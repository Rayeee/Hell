...
public class WebAPISecurityFilter implements Filter {
    ...
    private void handleException(HttpServletResponse response, Throwable throwable){
        try {
        	...
        	if(throwable.getCause() instanceof HellServiceException){
                HellServiceException hellServiceException = (HellServiceException) throwable.getCause();
        		responseEntity = new ResponseEntity(hellServiceException.getErrorCode(), hellServiceException.getMessage(), null);
        	}else {
                logger.error("UNKNOWN_ERROR ====", throwable);
